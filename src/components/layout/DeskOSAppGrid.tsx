...
export const DeskOSAppGrid: React.FC<DeskOSAppGridProps> = ({
  ...
  return (
    <div className="w-full max-w-5xl mx-auto py-2 select-none animate-fadeIn">
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3.5 sm:gap-5 justify-items-center">
        {apps.map((app, index) => {
          const Icon = app.icon;
          ...
          return (
            ...
            >
              ...
              <div 
                className={`w-13.5 h-13.5 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center shadow-[0_6px_15px_rgba(0,0,0,0.08)] border border-white/70 transition-transform duration-200 group-hover:shadow-[0_10px_20px_rgba(0,0,0,0.12)] ${
                  app.bg || 'bg-slate-200'
                } ${isActive ? 'ring-4 ring-amber-400 ring-offset-2' : ''}`}
              >
                <Icon className={`w-7 h-7 sm:w-8 sm:h-8 ${app.text}`} />
              </div>

              {/* App Label - Large, Bold & Extremely Legible */}
              <span className={`font-black text-xs sm:text-[13px] tracking-tight text-center max-w-[105px] sm:max-w-[115px] whitespace-normal line-clamp-2 leading-snug break-words min-h-[2.4rem] flex items-center justify-center transition-colors drop-shadow-2xs ${
                activeWallpaper?.appLabelTextColor || 'text-[#3d2b17]'
              }`}>
                {app.label}
              </span>
            </div>
