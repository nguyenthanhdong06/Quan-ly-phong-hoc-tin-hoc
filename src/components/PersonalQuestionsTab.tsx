...
const PREDEFINED_TOPICS_BY_GRADE: Record<number, Record<string, { group: string; topics: string[] }[]>> = {
  1: {
    'Toán': [
      {
...
const getTopicsByGradeAndSubject = (gradeId: number, subjectName: string) => {
  const gradeData = PREDEFINED_TOPICS_BY_GRADE[gradeId];
  if (!gradeData) return [];
  const matchedKey = Object.keys(gradeData).find(
    k => k.toLowerCase().trim() === subjectName.toLowerCase().trim()
  );
  if (matchedKey) return gradeData[matchedKey];
  if (gradeData['Tin học']) return gradeData['Tin học'];
  const keys = Object.keys(gradeData);
  if (keys.length > 0) return gradeData[keys[0]];
  return [];
};
...
export function PersonalQuestionsTab({ currentUser, showToast, selectedGrade = 3 }: PersonalQuestionsTabProps) {
  ...
  const formGradeTopics = useMemo(() => {
    const subj = subjects.find(s => s.id === formSubjectId);
    const subjName = subj ? subj.name : 'Tin học';
    return getTopicsByGradeAndSubject(formGradeId, subjName);
  }, [formGradeId, formSubjectId, subjects]);
  ...
  const importSubjectName = useMemo(() => {
    const subj = subjects.find(s => s.id === importSubjectId) || selectedSubject;
    return subj ? subj.name : 'Tin học';
  }, [subjects, importSubjectId, selectedSubject]);

  const importGradeTopics = useMemo(() => {
    return getTopicsByGradeAndSubject(importGradeId, importSubjectName);
  }, [importGradeId, importSubjectName]);

  useEffect(() => {
    const subj = subjects.find(s => s.id === importSubjectId) || selectedSubject;
    if (subj) {
      const newGrade = subj.gradeId;
      const gradeTopics = getTopicsByGradeAndSubject(newGrade, subj.name);
  ...
  const openAddQuestionModal = () => {
    ...
    const gradeTopics = getTopicsByGradeAndSubject(currentGrade, selectedSubject.name);
    const defaultCat = (gradeTopics[0] && gradeTopics[0].topics[0]) || '';
  ...
  const openEditQuestionModal = (q: Question) => {
    ...
    const qSubj = subjects.find(s => s.id === (q.subjectId || selectedSubjectId));
    const qSubjName = qSubj ? qSubj.name : 'Tin học';
    const gradeTopics = getTopicsByGradeAndSubject(currentGrade, qSubjName);
              ...
              <>
                ...
                <div className="bg-white p-5 rounded-3xl border border-slate-150 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  ...
                  <button
                    ...
                    className="btn-3d btn-3d-amber text-xs px-4 py-2.5 shadow-sm"
                  >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-12 gap-2 flex-1">
                        <div className="md:col-span-6 relative flex items-center">
                          ...
                          <input
                            ...
                            autoComplete="off"
                            spellCheck={false}
                            className="w-full bg-slate-50 border border-slate-200 hover:border-slate-350 rounded-xl pl-8 pr-8 py-1.5 text-xs font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-orange-500/10 focus:border-orange-500 focus:bg-white transition-all shadow-inner placeholder:text-slate-400"
                          />
                          {searchText && (
                            <button
                              type="button"
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-200">
                ...
                <div>
                  ...
                  >
                    ...
                    {importGradeTopics.map((group, gIdx) => (
                  ...
                  {(isCustomImportCategory || !importGradeTopics.flatMap(g => g.topics).includes(importCategory)) && (
                    <input
                      type="text"
                      required
                ...
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const currentSubjName = firstSubj ? firstSubj.name : 'Tin học';
                      // Auto update category to first topic in the new grade
                      const gradeTopics = getTopicsByGradeAndSubject(newGrade, currentSubjName);
                      const defaultCat = (gradeTopics[0] && gradeTopics[0].topics[0]) || '';
                      setFormCategory(defaultCat);
                      setIsCustomCategory(false);
                    }}
              ...
              <div className="space-y-1">
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    {formGradeTopics.map((group, gIdx) => (
                      <optgroup key={gIdx} label={group.group}>
                        {group.topics.map((topic, tIdx) => (
                          <option key={tIdx} value={topic}>{topic}</option>
                        ))}
                  ...
                  {(isCustomCategory || !formGradeTopics.flatMap(g => g.topics).includes(formCategory)) && (
                    <input
                      type="text"
                      required
                      value={formCategory}
